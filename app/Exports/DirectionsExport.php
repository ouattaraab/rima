...
class DirectionsExport implements FromCollection, WithHeadings, WithStyles, WithColumnWidths, WithTitle, WithEvents, WithDrawings, WithCustomStartCell
{
    ...
    {
        return Direction::orderBy('name')->get()->map(function ($item, $index) {
            return [
                $index + 1,
                $item->code,
                $item->name,
    ...
    {
        return ['#', 'Code', 'Nom', 'Actif', 'Date création'];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('B2', 'Liste des directions');
                ...
                for ($row = $firstData; $row <= $rowCount; $row++) {
                    ...
                    // Actif column (D)
                    $actifValue = $sheet->getCell("D{$row}")->getValue();
                ...
                $sheet->setCellValue("B{$totalRow}", "Total : {$dataCount} direction(s)");
