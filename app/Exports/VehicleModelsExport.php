...
class VehicleModelsExport implements FromCollection, WithHeadings, WithStyles, WithColumnWidths, WithTitle, WithEvents, WithDrawings, WithCustomStartCell
{
    ...
    {
        return VehicleModel::with('brand')->orderBy('name')->get()->map(function ($item, $index) {
            return [
                $index + 1,
                $item->brand?->name,
                $item->name,
    ...
    {
        return ['#', 'Marque', 'Nom', 'Catégorie', 'Actif', 'Date création'];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('B2', 'Liste des modèles de véhicules');
                ...
                $sheet->setCellValue("B{$totalRow}", "Total : {$dataCount} modèle(s)");
